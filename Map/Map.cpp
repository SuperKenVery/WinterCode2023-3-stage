...
Map::Map(vector<render_callback *> *render_objects, Renderer *renderer,
    ...
    /* 16x9
        ...
        2-soft wall
    */
    ifstream map_file(RESOURCE_PATH "map.txt");
    stringstream mapfile_stream;
    mapfile_stream << map_file.rdbuf();
    string map(mapfile_stream.str());
    ...
    for (int i = 0; i < map.length(); i++) {
        if (map[i] == '\n') {
            if (map[last_index] != '#' && i - last_index > 1)
